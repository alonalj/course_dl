...
def get_row_col_label(label, c, is_rows):
    label = int(label)
    if c.tiles_per_dim == 2:
        if is_rows:
            if label in [0,1]:
                label = 0
            else:
...
def data_generator(data_type, tiles_per_dim, data_split_dict, batch_size, c):
    ...
    for folder in folders:
        ...
        for f in files:
            ...
            label = get_row_col_label(label, c, True)
        ...
        if np.array(images_in_folder).shape != (c.n_original_tiles, c.max_size, c.max_size):
            continue
        ...
        folder_labels = to_categorical(labels_in_folder, num_classes=c.tiles_per_dim)
        y_batch.append(folder_labels)
        if len(y_batch) == batch_size:
            # print(np.array(X_batch).ndim)
            # print(np.array(X_batch))
            if np.array(X_batch).shape[1:] != (c.n_original_tiles, c.max_size, c.max_size, 2):
                print(folder)
                print(np.array(X_batch).shape)
            yield list(np.array(X_batch).reshape(c.n_original_tiles, batch_size, c.max_size, c.max_size, 2)), \
                  list(np.array(y_batch).reshape(c.n_original_tiles, batch_size, c.tiles_per_dim))
            X_batch = []
            y_batch = []
    ...
    if len(y_batch) != batch_size:  # if equal, already yielded above
        ...
        if np.array(X_batch).shape[1:] != (c.n_original_tiles, c.max_size, c.max_size, 2):
            print(folder)
            print(np.array(X_batch).shape)
        yield list(np.array(X_batch).reshape(c.n_original_tiles, -1, c.max_size, c.max_size, 2)), \
              list(np.array(y_batch).reshape(c.n_original_tiles, -1, c.tiles_per_dim))
...
def run(c):
    ...
    resnet = build_resnet_rows_cols(c.max_size, c.n_original_tiles, c.tiles_per_dim, c.n_original_tiles, c.tiles_per_dim)
    ...
    for e in range(maxepoches):
        ...
        for X_batch_val, y_batch_val in val_generator:
            ...
            current_acc.append(np.mean(hist_val[-c.n_original_tiles:]))
        current_avg_acc = np.mean(current_acc)
        if current_avg_acc > best_avg_acc_val:
            resnet.save_weights(
                'rows_cols_resnet_maxSize_{}_tilesPerDim_{}_nTilesPerSample_{}_isImg_{}_mID_{}_L_{}.h5'.format(c.max_size,
                                                                                                     c.tiles_per_dim,
                                                                                                     c.n_original_tiles,
                                                                                                     c.is_images,
                                                                                                     c.mID,
